  ...
  if (isSummaryReport) {
    ...
    if (trPercValue !== null) {
      ...
      if (trPercValue < TROUBLE_PERCENTAGE_THRESHOLD) {
        ...
        const alertMessage = `<@${process.env.SLACK_OWNER_USER_SAVA_ID}> <@${process.env.SLACK_OWNER_USER_GORYAINOV_ID}> Візьміть, будь ласка, в роботу - прийшло відхилееня >7%.`;
...
function sendMissingReportNotification(reportType, channel) {
  let message = `<@${process.env.SLACK_DEV_USER_BONDARENKO_ID}> Не прийшов звіт ${reportType}, перевірте, будь ласка, в чому причина. Для інформації: <@${process.env.SLACK_OWNER_USER_SAVA_ID}> <@${process.env.SLACK_OWNER_USER_GORYAINOV_ID}>`;
